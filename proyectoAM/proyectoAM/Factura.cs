...
<<<<<<< HEAD
        public void guarda()
        {
            ...
            if (itm.agregarItems(new string[] { nudCantidad.Text, cbDescripcion.Text, txtPrecio.Text }))
            {
            ...
            {
                conecta();
                string sql2 = "SELECT id FROM items WHERE descripcion='" + cbDescripcion.Text + "';";
                cmd = new MySqlCommand(sql2, cn);
                reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    idite = (reader["id"].ToString());
            ...
            if (fact.agregarFactura(new string[] { txtidcli.Text, idite, cbPago.SelectedItem.ToString(), vence }))
            {
                // MessageBox.Show("Agregado a Base de datos");
            }
        }
...
>>>>>>> origin/master
      ...
      (e.KeyChar != '.'))
        ...
        public void exporta()
        {
            ...
            if (string.IsNullOrEmpty(cbNombre.Text)) { PdfPCell parag = new PdfPCell(new Phrase( ("CLIENTE: "  + cbNombre.SelectedItem.ToString() + " " + txtrnccom.Text), cliente)); parag.Border = 0; parag.HorizontalAlignment = 0; parag.Colspan = 2; adjust.AddCell(parag); }
            else { PdfPCell parag = new PdfPCell(new Phrase ("CLIENTE: " + cbNombre.Text.ToString() + " " +txtrnccom.Text , cliente)); parag.Border = 0; parag.HorizontalAlignment = 0; parag.Colspan = 2; adjust.AddCell(parag); }
            doc.Add(adjust);
...
<<<<<<< HEAD
        ...
        {
           /* string nom = cbNombre.SelectedItem.ToString();
            cli.mostrarrnc(nom);
